...
export class Orchestrator {
  ...
  private readonly debateStore: InMemoryDebateStore;
  private readonly debateCache = new Map<string, ToolResult>();
  ...
  async execute(command: ToolCommand, input: ToolInput): Promise<ToolResult> {
    ...
    const promptResult = sanitizePrompt(
      normalizePrompt(command, input),
      input.constraints?.max_prompt_chars ?? 12000,
    );
    ...
    const { result, selected, errors, fallbackTried } = await fallbackExecutor.run(
      attempts,
      async (bridge, attemptNo, fallbackFrom) => {
        ...
        try {
          ...
          if (
            typeof input.constraints?.max_latency_ms === "number" &&
  ...
  private async executeHybridDebate(
    input: ToolInput,
    requestId: string,
    sessionId: string,
  ): Promise<ToolResult> {
    ...
    for (const [index, participant] of participants.entries()) {
      ...
      try {
        ...
        const fallbackBridge = this.registry
          .list()
          .find(
            (bridge) => !used.has(bridge) && !reservedParticipants.has(bridge) && health[bridge],
          );

        if (!fallbackBridge) {
          throw new CoreError(
            "TOOL_ERROR",
            `debate participant ${participant} failed without fallback`,
          );
        }
      ...
      this.sessionStore.setThread(
        sessionId,
        selectedBridge,
        `${selectedBridge}_${requestId}_${index + 1}`,
      );
...
function evaluateDebate(responses: BridgeResponse[]): {
...
function normalizePrompt(command: ToolCommand, input: ToolInput): string {
  if (command === "debate") {
...
function toTokenUsage(inputTokens: number, outputTokens: number): TokenUsage {
  return {
    inputTokens,
    outputTokens,
...
function sumTokenUsage(responses: BridgeResponse[]): TokenUsage {
