...
class CliBridgeAdapter {
    ...
    async send(input) {
        ...
        for (const [index, model] of this.modelChain.entries()) {
            ...
            try {
                const { stdout, stderr } = await runCommand(this.config.command, this.config.buildArgs(prompt, model), DEFAULT_TIMEOUT_MS, DEFAULT_MAX_OUTPUT_BYTES);
...
export function createRealCliAdapters() {
...
export function shouldUseRealCliAdapters() {
...
export function parseClaudeJson(stdout) {
    ...
    const usage = typeof parsed.usage === "object" && parsed.usage
        ? parsed.usage
        : {};
    return {
...
export function parseGeminiJson(stdout) {
    ...
    if (typeof parsed.stats === "object" && parsed.stats) {
        ...
        const models = typeof stats.models === "object" && stats.models
            ? stats.models
            : {};
        let promptTotal = 0;
...
async function runCommand(command, args, timeoutMs, maxOutputBytes) {
...
function defaultModelUnavailableMatcher(message) {
    return isLikelyModelUnavailableError(message);
}
function isLikelyModelUnavailableError(message) {
...
//# sourceMappingURL=real-cli-adapters.js.map